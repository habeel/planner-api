...
export class PlanningService {
    ...
    async getWeekPlanning(workspaceId, weekStart) {
        const { start, end } = getWeekRange(weekStart);
        const startStr = start.toISOString().split('T')[0];
        const endStr = end.toISOString().split('T')[0];
        // Get all users in workspace with their capacity
        const usersResult = await this.fastify.db.query(`SELECT u.id, u.email, u.name, u.capacity_week_hours
       FROM users u
       ...
       WHERE uwr.workspace_id = $1 AND u.is_active = true`, [workspaceId]);
        ...
        const tasksResult = await this.fastify.db.query(`SELECT * FROM tasks
       WHERE workspace_id = $1
       AND (start_date BETWEEN $2 AND $3 OR start_date IS NULL)
       ORDER BY COALESCE(start_date, '9999-12-31'), priority DESC`, [workspaceId, startStr, endStr]);
        ...
        const timeOffResult = await this.fastify.db.query(`SELECT user_id,
       ...
       GROUP BY user_id`, [workspaceId, startStr, endStr]);
    ...
    async getMonthPlanning(workspaceId, month) {
    ...
    async autoSchedule(workspaceId, startDate, endDate, _strategy = 'greedy') {
       ...
       AND status != 'DONE'
       ORDER BY priority DESC, created_at`, [workspaceId]);
