...
export default async function integrationRoutes(fastify) {
    ...
    async function checkAdminAccess(workspaceId, userId, reply) {
    ...
    async function checkWorkspaceAccess(workspaceId, userId, reply) {
        const role = await workspaceService.getUserRole(workspaceId, userId);
        if (!role) {
    ...
    fastify.get('/:workspaceId', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    fastify.post('/github/setup', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    fastify.post('/github/test', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const parseResult = githubConfigSchema.safeParse(request.body);
        if (!parseResult.success) {
            return reply.status(400).send({
                error: 'Invalid input',
    ...
    fastify.delete('/github/:workspaceId', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { workspaceId } = request.params;
        if (!(await checkAdminAccess(workspaceId, request.user.id, reply))) {
            return;
        }
    ...
    fastify.patch('/github/:workspaceId/toggle', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { workspaceId } = request.params;
        ...
        const updated = await integrationService.updateIntegrationEnabled(workspaceId, 'github', parseResult.data.enabled);
        if (!updated) {
    ...
    fastify.post('/github/search', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    fastify.post('/jira/setup', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        const config = { baseUrl, projectKey };
        const credentials = { pat, email: '' }; // email not needed for PAT
        // Test the connection first
    ...
    fastify.post('/jira/test', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const parseResult = jiraConfigSchema.safeParse(request.body);
        if (!parseResult.success) {
            return reply.status(400).send({
                error: 'Invalid input',
        ...
        const credentials = { pat, email: '' };
        const testResult = await integrationService.testJiraConnection(config, credentials);
        return reply.send(testResult);
    });
    // DELETE /api/integrations/jira/:workspaceId - Remove Jira integration
    fastify.delete('/jira/:workspaceId', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { workspaceId } = request.params;
        if (!(await checkAdminAccess(workspaceId, request.user.id, reply))) {
            return;
        }
    ...
    fastify.patch('/jira/:workspaceId/toggle', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { workspaceId } = request.params;
        const parseResult = z.object({ enabled: z.boolean() }).safeParse(request.body);
        if (!parseResult.success) {
            return reply.status(400).send({
        ...
        const updated = await integrationService.updateIntegrationEnabled(workspaceId, 'jira', parseResult.data.enabled);
        if (!updated) {
            return reply.status(404).send({
    ...
    fastify.post('/jira/search', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        const config = integration.config;
        const credentials = integration.credentials;
        const issues = await integrationService.searchJiraIssues(config, credentials, query, projectKey || config.projectKey);
        return reply.send({ issues });
    });
    // POST /api/integrations/links/jira - Link a Jira issue to a task
    fastify.post('/links/jira', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        // Create the link
        const link = await integrationService.addTaskLink(taskId, 'jira', issue.key, integrationService.getJiraIssueUrl(config.baseUrl, issue.key), issue.fields.summary, integrationService.getJiraStatusCategory(issue));
        return reply.status(201).send({ link, issue });
    });
    // ============ Task External Links ============
    // GET /api/integrations/links/:taskId - Get all external links for a task
    fastify.get('/links/:taskId', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    fastify.post('/links/github', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        // Create the link
        const link = await integrationService.addTaskLink(taskId, 'github', String(issue.number), issue.html_url, issue.title, issue.state);
        return reply.status(201).send({ link, issue });
    });
    // POST /api/integrations/links - Add a manual external link
    fastify.post('/links', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        const link = await integrationService.addTaskLink(taskId, provider, externalId, externalUrl, title, status);
        return reply.status(201).send({ link });
    });
    // DELETE /api/integrations/links/:taskId/:linkId - Remove an external link
    fastify.delete('/links/:taskId/:linkId', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    fastify.post('/links/:taskId/sync', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { taskId } = request.params;
    ...
    fastify.post('/links/bulk', { preHandler: [fastify.authenticate] }, async (request, reply) => {
