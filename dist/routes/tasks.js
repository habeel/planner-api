...
export default async function taskRoutes(fastify) {
    ...
    async function checkWorkspaceAccess(workspaceId, userId, reply, requireWrite = false) {
    ...
    fastify.patch('/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        try {
            ...
            if (err.message.includes('Cannot edit fields')) {
                return reply.status(400).send({
                    error: err.message,
                    code: 'EXTERNAL_FIELD_EDIT_BLOCKED',
                });
            }
            if (err.message.includes('Estimated hours must be greater than 0')) {
                return reply.status(400).send({
                    error: err.message,
    ...
    fastify.post('/:id/dependencies', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { id } = request.params;
        ...
        try {
            const dependency = await taskService.addDependency(id, parseResult.data.depends_on_task_id, parseResult.data.type);
    ...
    fastify.delete('/:id/dependencies/:depId', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    fastify.post('/dependencies/batch', { preHandler: [fastify.authenticate] }, async (request, reply) => {
