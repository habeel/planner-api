...
export class TaskService {
  ...
  async update(id: string, input: UpdateTaskInput, currentTask: Task): Promise<Task | null> {
    ...
    const plannedStatuses: TaskStatus[] = ['PLANNED', 'IN_PROGRESS', 'BLOCKED', 'DONE'];
  ...
  async getTasksForWeek(workspaceId: string, weekStart: string): Promise<Task[]> {
  ...
  async addDependency(
    ...
    const result = await this.fastify.db.query<TaskDependency>(
  ...
  async getDependenciesForTasks(taskIds: string[]): Promise<TaskDependency[]> {
    ...
    const result = await this.fastify.db.query<TaskDependency>(
      `SELECT * FROM task_dependencies
       WHERE task_id IN (${placeholders1}) OR depends_on_task_id IN (${placeholders2})`,
      [...taskIds, ...taskIds]
    );
    return result.rows;
  }
  ...
  async hasCircularDependency(taskId: string, dependsOnTaskId: string): Promise<boolean> {
    ...
    const result = await this.fastify.db.query<{ has_cycle: boolean }>(
